...
public class FetchJSONDataService : IFetchJSONDataService
{
            ...
            try {
                ...
                if (targetGroup!=null&&targetGroup.Items!=null) {
                    ...
                    foreach (var item in targetGroup.Items) {
                        ...
                        item.ExpanderSwitchState=DefaultToggleSwitchStates.GetDefaultState(item.UniqueID!);
                        item.ExpanderSwitchVisibility=DefaultToggleSwitchStates.GetDefaultState(item.UniqueID!);
                    }
    ...
    /// </summary>
    /// <param name="registryGroupName">The UniqueID to search for within the settings groups.</param>
        ...
        using (HttpClient client = new HttpClient()) {
            try {
                ...
                if (settingsGroups!=null) {
                    ...
                    // TODO: Implement CheckIfCustomCommand
                    return;
                }
            }
            catch (Exception ex) {
    ...
    /// </summary>
    /// <param name="registryGroupName">UniqueID used to filter registry settings.</param>
